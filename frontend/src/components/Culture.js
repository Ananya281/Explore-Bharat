...
import "./Culture.css"; // Ensure this file is correctly linked
...
const Culture = ({ stateName }) => {
  ...
  useEffect(() => {
    ...
    const fetchCultureSection = async () => {
      try {
        ...
        if (!response.ok) {
          setContent("Failed to load cultural information.");
          return;
        }
        ...
        const cultureHeader = Array.from(
          doc.querySelectorAll("h2, h3")
        ).find((header) =>
          header.textContent.toLowerCase().includes("culture")
        );

        if (cultureHeader) {
          const cultureContent = [];
          let nextSibling = cultureHeader.nextElementSibling;
          while (nextSibling && nextSibling.tagName !== "H2") {
            cultureContent.push(nextSibling.outerHTML);
            nextSibling = nextSibling.nextElementSibling;
          }

          // Join the extracted content
          let fullContent = cultureContent.join("");
          ...
          setContent(
            filteredContent || "No detailed Cultural information available."
          );
        } else {
          setContent("No Culture section found.");
        }
      } catch (error) {
  ...
  useEffect(() => {
    if (isExpanded) {
      ...
      setDisplayedContent(
        content.substring(0, 2400) + (content.length > 300 ? "..." : "")
      );
    }
  }, [content, isExpanded]);
  ...
  return (
    <section className="relative culture-section py-16 bg-[#f3ece4] text-center overflow-hidden">
      {/* Decorative Patterns */}
      <div
      ...
      <h2 className="text-4xl font-bold text-[#6b4226] mb-8">
        Culture & Tradition of {formattedStateName}
      </h2>
      <div
